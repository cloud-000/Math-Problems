...
/**
 * Get the node at a given path string, starting from a root FSFolder node.
 * @param {FSFolder} root - The root folder node (where "~" points to).
 * @param {string} pathStr - The path string, e.g. "~/Folder1/Test/a"
 * @returns {CFileSystemNode|null} The found node or null if not found.
 */
function getNodeAtPath(root, pathStr) {
    ...
    if (path.startsWith(root.name + root.separator)) {
        path = path.slice(2);
    } else if (path === root.name) {
        return root;
    } else if (path.startsWith(root.separator)) {
        path = path.slice(1);
    }
    if (!path) return root;

    let parts = path.split(root.separator);
    let node = root;
